...
def h1a_t2a_c0(O, h, t, gamma1, eta1, lambdas, orbspace, verbose=False, scale=1.0):
	...
	O['0'] += scale * -1.00000000 * np.einsum('iu,vwux,vwix->', h['a'][c,a], lambda2['aa'], t['aa'][pa,pa,hc,ha], optimize=True)
	O['0'] += scale * -1.00000000 * np.einsum('ua,uvwx,vawx->', h['a'][a,v], lambda2['aa'], t['aa'][pa,pv,ha,ha], optimize=True)
...
def h1b_t2c_c0(O, h, t, gamma1, eta1, lambdas, orbspace, verbose=False, scale=1.0):
	...
	O['0'] += scale * -1.00000000 * np.einsum('IU,VWUX,VWIX->', h['b'][C,A], lambda2['bb'], t['bb'][pA,pA,hC,hA], optimize=True)
	O['0'] += scale * -1.00000000 * np.einsum('UA,UVWX,VAWX->', h['b'][A,V], lambda2['bb'], t['bb'][pA,pV,hA,hA], optimize=True)
...
def h2a_t1a_c0(O, h, t, gamma1, eta1, lambdas, orbspace, verbose=False, scale=1.0):
	...
	O['0'] += scale * +1.00000000 * np.einsum('iuvw,uxvw,xi->', h['aa'][c,a,a,a], lambda2['aa'], t['a'][pa,hc], optimize=True)
	O['0'] += scale * +1.00000000 * np.einsum('uvwa,uvwx,ax->', h['aa'][a,a,a,v], lambda2['aa'], t['a'][pv,ha], optimize=True)
...
def h2a_t2a_c0(O, h, t, gamma1, eta1, lambdas, orbspace, verbose=False, scale=1.0):
	...
	O['0'] += scale * +2.00000000 * np.einsum('uv,wx,yz,iyvx,uwiz->', eta1['a'], eta1['a'], gamma1['a'], h['aa'][c,a,a,a], t['aa'][pa,pa,hc,ha], optimize=True)
	O['0'] += scale * +0.25000000 * np.einsum('uv,wx,ijvx,uwij->', eta1['a'], eta1['a'], h['aa'][c,c,a,a], t['aa'][pa,pa,hc,hc], optimize=True)
	
	
	O['0'] += scale * +2.00000000 * np.einsum('uv,wx,yz,wyva,uaxz->', eta1['a'], gamma1['a'], gamma1['a'], h['aa'][a,a,a,v], t['aa'][pa,pv,ha,ha], optimize=True)
	O['0'] += scale * +16.00000000 * np.einsum('uv,wx,iwva,uaix->', eta1['a'], gamma1['a'], h['aa'][c,a,a,v], t['aa'][pa,pv,hc,ha], optimize=True)
	
	
	O['0'] += scale * +2.00000000 * np.einsum('uv,ijva,uaij->', eta1['a'], h['aa'][c,c,a,v], t['aa'][pa,pv,hc,hc], optimize=True)
	O['0'] += scale * +4.00000000 * np.einsum('uv,iwvx,wyxz,uyiz->', eta1['a'], h['aa'][c,a,a,a], lambda2['aa'], t['aa'][pa,pa,hc,ha], optimize=True)
	
	
	O['0'] += scale * +1.00000000 * np.einsum('uv,wxva,wxyz,uayz->', eta1['a'], h['aa'][a,a,a,v], lambda2['aa'], t['aa'][pa,pv,ha,ha], optimize=True)
	
	O['0'] += scale * +0.25000000 * np.einsum('uv,wx,uwab,abvx->', gamma1['a'], gamma1['a'], h['aa'][a,a,v,v], t['aa'][pv,pv,ha,ha], optimize=True)
	
	O['0'] += scale * +1.00000000 * np.einsum('uv,iuwx,yzwx,yziv->', gamma1['a'], h['aa'][c,a,a,a], lambda2['aa'], t['aa'][pa,pa,hc,ha], optimize=True)
	O['0'] += scale * +2.00000000 * np.einsum('uv,iuab,abiv->', gamma1['a'], h['aa'][c,a,v,v], t['aa'][pv,pv,hc,ha], optimize=True)
	
	O['0'] += scale * +4.00000000 * np.einsum('uv,uwxa,wyxz,yavz->', gamma1['a'], h['aa'][a,a,a,v], lambda2['aa'], t['aa'][pa,pv,ha,ha], optimize=True)
	...
	O['0'] += scale * +1.00000000 * np.einsum('iuvw,uxyvwz,xyiz->', h['aa'][c,a,a,a], lambda3['aaa'], t['aa'][pa,pa,hc,ha], optimize=True)
	O['0'] += scale * +16.00000000 * np.einsum('iuva,uwvx,waix->', h['aa'][c,a,a,v], lambda2['aa'], t['aa'][pa,pv,hc,ha], optimize=True)
	O['0'] += scale * -1.00000000 * np.einsum('uvwa,uvxwyz,xayz->', h['aa'][a,a,a,v], lambda3['aaa'], t['aa'][pa,pv,ha,ha], optimize=True)
...
def h2a_t2b_c0(O, h, t, gamma1, eta1, lambdas, orbspace, verbose=False, scale=1.0):
	...
	O['0'] += scale * +2.00000000 * np.einsum('uv,iwvx,wUxV,uUiV->', eta1['a'], h['aa'][c,a,a,a], lambda2['ab'], t['ab'][pa,pA,hc,hA], optimize=True)
	
	
	
	O['0'] += scale * -2.00000000 * np.einsum('uv,uwxa,wUxV,aUvV->', gamma1['a'], h['aa'][a,a,a,v], lambda2['ab'], t['ab'][pv,pA,ha,hA], optimize=True)
	
	O['0'] += scale * +1.00000000 * np.einsum('iuvw,uxUvwV,xUiV->', h['aa'][c,a,a,a], lambda3['aab'], t['ab'][pa,pA,hc,hA], optimize=True)
	O['0'] += scale * -4.00000000 * np.einsum('iuva,uUvV,aUiV->', h['aa'][c,a,a,v], lambda2['ab'], t['ab'][pv,pA,hc,hA], optimize=True)
	O['0'] += scale * +1.00000000 * np.einsum('uvwa,uvUwxV,aUxV->', h['aa'][a,a,a,v], lambda3['aab'], t['ab'][pv,pA,ha,hA], optimize=True)
...
def h2b_t2a_c0(O, h, t, gamma1, eta1, lambdas, orbspace, verbose=False, scale=1.0):
	...
	O['0'] += scale * +2.00000000 * np.einsum('uv,iUvV,wUxV,uwix->', eta1['a'], h['ab'][c,A,a,A], lambda2['ab'], t['aa'][pa,pa,hc,ha], optimize=True)
	
	
	
	O['0'] += scale * -2.00000000 * np.einsum('uv,uUaV,wUxV,wavx->', gamma1['a'], h['ab'][a,A,v,A], lambda2['ab'], t['aa'][pa,pv,ha,ha], optimize=True)
	
	O['0'] += scale * -1.00000000 * np.einsum('iUuV,vwUuxV,vwix->', h['ab'][c,A,a,A], lambda3['aab'], t['aa'][pa,pa,hc,ha], optimize=True)
	O['0'] += scale * -4.00000000 * np.einsum('iUaV,uUvV,uaiv->', h['ab'][c,A,v,A], lambda2['ab'], t['aa'][pa,pv,hc,ha], optimize=True)
	O['0'] += scale * -1.00000000 * np.einsum('uUaV,uvUwxV,vawx->', h['ab'][a,A,v,A], lambda3['aab'], t['aa'][pa,pv,ha,ha], optimize=True)
...
def h2b_t2c_c0(O, h, t, gamma1, eta1, lambdas, orbspace, verbose=False, scale=1.0):
	...
	O['0'] += scale * +2.00000000 * np.einsum('UV,uIvV,uWvX,UWIX->', eta1['b'], h['ab'][a,C,a,A], lambda2['ab'], t['bb'][pA,pA,hC,hA], optimize=True)
	
	
	
	O['0'] += scale * -2.00000000 * np.einsum('UV,uUvA,uWvX,WAVX->', gamma1['b'], h['ab'][a,A,a,V], lambda2['ab'], t['bb'][pA,pV,hA,hA], optimize=True)
	
	O['0'] += scale * -1.00000000 * np.einsum('uIvU,uVWvUX,VWIX->', h['ab'][a,C,a,A], lambda3['abb'], t['bb'][pA,pA,hC,hA], optimize=True)
	O['0'] += scale * -4.00000000 * np.einsum('uIvA,uUvV,UAIV->', h['ab'][a,C,a,V], lambda2['ab'], t['bb'][pA,pV,hC,hA], optimize=True)
	O['0'] += scale * -1.00000000 * np.einsum('uUvA,uUVvWX,VAWX->', h['ab'][a,A,a,V], lambda3['abb'], t['bb'][pA,pV,hA,hA], optimize=True)
...
def h2c_t1b_c0(O, h, t, gamma1, eta1, lambdas, orbspace, verbose=False, scale=1.0):
	...
	O['0'] += scale * +1.00000000 * np.einsum('IUVW,UXVW,XI->', h['bb'][C,A,A,A], lambda2['bb'], t['b'][pA,hC], optimize=True)
	O['0'] += scale * +1.00000000 * np.einsum('UVWA,UVWX,AX->', h['bb'][A,A,A,V], lambda2['bb'], t['b'][pV,hA], optimize=True)
...
def h2c_t2b_c0(O, h, t, gamma1, eta1, lambdas, orbspace, verbose=False, scale=1.0):
	...
	O['0'] += scale * +2.00000000 * np.einsum('UV,IWVX,uWvX,uUvI->', eta1['b'], h['bb'][C,A,A,A], lambda2['ab'], t['ab'][pa,pA,ha,hC], optimize=True)
	
	
	
	O['0'] += scale * -2.00000000 * np.einsum('UV,UWXA,uWvX,uAvV->', gamma1['b'], h['bb'][A,A,A,V], lambda2['ab'], t['ab'][pa,pV,ha,hA], optimize=True)
	
	O['0'] += scale * +1.00000000 * np.einsum('IUVW,uUXvVW,uXvI->', h['bb'][C,A,A,A], lambda3['abb'], t['ab'][pa,pA,ha,hC], optimize=True)
	O['0'] += scale * -4.00000000 * np.einsum('IUVA,uUvV,uAvI->', h['bb'][C,A,A,V], lambda2['ab'], t['ab'][pa,pV,ha,hC], optimize=True)
	O['0'] += scale * +1.00000000 * np.einsum('UVWA,uUVvWX,uAvX->', h['bb'][A,A,A,V], lambda3['abb'], t['ab'][pa,pV,ha,hA], optimize=True)
...
def h2c_t2c_c0(O, h, t, gamma1, eta1, lambdas, orbspace, verbose=False, scale=1.0):
	...
	O['0'] += scale * +2.00000000 * np.einsum('UV,WX,YZ,IYVX,UWIZ->', eta1['b'], eta1['b'], gamma1['b'], h['bb'][C,A,A,A], t['bb'][pA,pA,hC,hA], optimize=True)
	O['0'] += scale * +0.25000000 * np.einsum('UV,WX,IJVX,UWIJ->', eta1['b'], eta1['b'], h['bb'][C,C,A,A], t['bb'][pA,pA,hC,hC], optimize=True)
	
	
	O['0'] += scale * +2.00000000 * np.einsum('UV,WX,YZ,WYVA,UAXZ->', eta1['b'], gamma1['b'], gamma1['b'], h['bb'][A,A,A,V], t['bb'][pA,pV,hA,hA], optimize=True)
	O['0'] += scale * +16.00000000 * np.einsum('UV,WX,IWVA,UAIX->', eta1['b'], gamma1['b'], h['bb'][C,A,A,V], t['bb'][pA,pV,hC,hA], optimize=True)
	
	
	O['0'] += scale * +2.00000000 * np.einsum('UV,IJVA,UAIJ->', eta1['b'], h['bb'][C,C,A,V], t['bb'][pA,pV,hC,hC], optimize=True)
	O['0'] += scale * +4.00000000 * np.einsum('UV,IWVX,WYXZ,UYIZ->', eta1['b'], h['bb'][C,A,A,A], lambda2['bb'], t['bb'][pA,pA,hC,hA], optimize=True)
	
	
	O['0'] += scale * +1.00000000 * np.einsum('UV,WXVA,WXYZ,UAYZ->', eta1['b'], h['bb'][A,A,A,V], lambda2['bb'], t['bb'][pA,pV,hA,hA], optimize=True)
	
	O['0'] += scale * +0.25000000 * np.einsum('UV,WX,UWAB,ABVX->', gamma1['b'], gamma1['b'], h['bb'][A,A,V,V], t['bb'][pV,pV,hA,hA], optimize=True)
	
	O['0'] += scale * +1.00000000 * np.einsum('UV,IUWX,YZWX,YZIV->', gamma1['b'], h['bb'][C,A,A,A], lambda2['bb'], t['bb'][pA,pA,hC,hA], optimize=True)
	O['0'] += scale * +2.00000000 * np.einsum('UV,IUAB,ABIV->', gamma1['b'], h['bb'][C,A,V,V], t['bb'][pV,pV,hC,hA], optimize=True)
	
	O['0'] += scale * +4.00000000 * np.einsum('UV,UWXA,WYXZ,YAVZ->', gamma1['b'], h['bb'][A,A,A,V], lambda2['bb'], t['bb'][pA,pV,hA,hA], optimize=True)
	...
	O['0'] += scale * +1.00000000 * np.einsum('IUVW,UXYVWZ,XYIZ->', h['bb'][C,A,A,A], lambda3['bbb'], t['bb'][pA,pA,hC,hA], optimize=True)
	O['0'] += scale * +16.00000000 * np.einsum('IUVA,UWVX,WAIX->', h['bb'][C,A,A,V], lambda2['bb'], t['bb'][pA,pV,hC,hA], optimize=True)
	O['0'] += scale * -1.00000000 * np.einsum('UVWA,UVXWYZ,XAYZ->', h['bb'][A,A,A,V], lambda3['bbb'], t['bb'][pA,pV,hA,hA], optimize=True)
