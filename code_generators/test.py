...
def compile_einsum(expression, fmt='dict', tensor_name=None, remove_lambda3=False, remove_lambda4=False):
    """
    ...
    """
    unused_indices = {'a':list('uvwxyzrsto'), 'A':list('UVWXYZRSTO'), 
...
def make_code(mbeq, fmt='dict', tensor_name=None, remove_lambda3=False, remove_lambda4=False):
    code = ''
    nlines = 0
    for i in mbeq:
        einsum = compile_einsum(i, fmt, tensor_name, remove_lambda3=remove_lambda3, remove_lambda4=remove_lambda4)
        code += '\t' + einsum + '\n'
        nlines += 1
    return code, nlines
...
def make_nbody_elements(op1, op2, nbody, spincase, fmt='slice', keys_in=None, do_lhs_slice=True, remove_lambda3=False, remove_lambda4=False):
    """
    ...
    for key in keys:
        ...
        if nbody == 3:
            if spincase == 'aaa':
                if num_alpha != 6: continue
            elif spincase == 'aab':
                if num_alpha != 4: continue
            elif spincase == 'abb':
                if num_alpha != 2: continue
            elif spincase == 'bbb':
                if num_beta != 6: continue
        ...
        if (nbody != 0 and do_lhs_slice):
            ...
            _ = make_code(mbeq[key], fmt, f"O['{spincase}'][{lhs_slice}]", remove_lambda3=remove_lambda3, remove_lambda4=remove_lambda4)
        else:
            _ = make_code(mbeq[key], fmt, f"O['{spincase}']", remove_lambda3=remove_lambda3, remove_lambda4=remove_lambda4)
        code += _[0]
        nlines += _[1]
    return code, nlines
...
if __name__ == "__main__":
    ...
    if remove_lambda4:
        print("WARNING: no 4-cumulant included!")
    ...
    nfiles = 0
    with open(f'c0.py', 'w') as f:
        for key in input_dict.keys():
            if 'c0' not in key: continue
            code, nlines = make_nbody_elements(*input_dict[key], remove_lambda3=remove_lambda3, remove_lambda4=remove_lambda4)
            print(key, nlines)
