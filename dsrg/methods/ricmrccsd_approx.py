...
from dsrg.wicked_contractions.ricmrccsd_approx_contractions import *
from dsrg.wicked_contractions.ricmrccsd_approx_hbar_active import *
from dsrg.wicked_contractions.ricmrccsd_approx_hbar import *
...
def compute_hbar(hamiltonian, T, ref, herm):

    # Initial value for the residual (0 commutators)
    X = {'a': ref.F['a'].copy(),
         'b': ref.F['b'].copy(),
         'aa': 0.25 * ref.V['aa'].copy(),
         'ab': ref.V['ab'].copy(),
         'bb': 0.25 * ref.V['bb'].copy()}
    
    # 1-body
    _t0 = time.time()
    X = Hbar_full_ncomm1_nbody1(X, hamiltonian, T, ref.gam1, ref.eta1, ref.lambdas, ref.orbspace)
    X = Hbar_full_ncomm2_nbody1(X, hamiltonian, T, ref.gam1, ref.eta1, ref.lambdas, ref.orbspace)
    # print(f"time for onebody {time.time() - _t0}")
    # 2-body
    _t0 = time.time()
    X = Hbar_full_ncomm1_nbody2(X, hamiltonian, T, ref.gam1, ref.eta1, ref.lambdas, ref.orbspace)
    X = Hbar_full_ncomm2_nbody2(X, hamiltonian, T, ref.gam1, ref.eta1, ref.lambdas, ref.orbspace)
    # print(f"time for twobody {time.time() - _t0}")
    # antisymmetrize twobody
    X['aa'] -= X['aa'].transpose(1, 0, 2, 3)
    X['aa'] -= X['aa'].transpose(0, 1, 3, 2)
